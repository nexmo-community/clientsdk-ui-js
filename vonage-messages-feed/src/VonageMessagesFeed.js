...
export class VonageMessagesFeed extends LitElement {
  static get styles() {
    return css`
      :host {
        display: block;
        padding: 25px;
        color: var(--vonage-messages-feed-text-color, #000);
      }
      #messages-container {
        width: 100%;
        height: 100%;
      ...
      <div id="messages-container">
        ${this.messages.map((message) => html`
          ${this.myId === message.message.from ?
            html`<div class="message my-message">${message.message.body.text.replace(/</g,"&lt;")}<br/><small>${message.sender.display_name.replace(/</g,"&lt;")}</small></div>`:
            html`<div class="message">${message.message.body.text.replace(/</g,"&lt;")}<br/><small>${message.sender.display_name.replace(/</g,"&lt;")}</small></div>`}
        `)}
      </div>
    `;
  }
